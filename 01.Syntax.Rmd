...
R has a few natively defined functions and many more defined through packages
...
The language R provides a mechanism to supply functions, variables, and data bundled as a package. Contributors write R packages and share them mainly via the CRAN repository. The base package comes with the basic R installation.
The `builtins()` function from the `base` R package lists the `r length(builtins())` predefined functions that come with R.

```{r number-functions-base-package, echo=T, message=FALSE, warning=FALSE}
# get the number of functions in the base R package
length(builtins())
