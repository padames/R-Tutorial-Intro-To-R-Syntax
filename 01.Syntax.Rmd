...
The operators `<-` and `=` act identically in stand alone expressions like the ones displayed above because they both affect the global environment by creating new name-value pairs. However when used to assign a value to a named parameter to a function, as in the `log(x, base = exp(1))` example above, `=` does not affect the global environment but it limits its action to the local variable for the scope of the function, in the example: it evaluates the function `exp` with argument `1` and it assigns its result to the local variable `base`. `log(x, base <- exp(1))` would do the same with he side effect of creating a variable `base` in the global environment, however the value is given to `base` by position because `<-` does not match function parameters by name. Thus if we wrote `log(base <- exp(1), x)` the value of evaluating `exp(1)` would be given to `x` by position, and the value of evaluating `x` from the global environment would be assigned to `base` by position, not really the desired intention. So to avoid ambiguities, use `<-` for stand-alone expressions and `=` for function named parameter assignment. 
