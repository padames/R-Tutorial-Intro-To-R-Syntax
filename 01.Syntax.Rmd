...
The operators `<-` and `=` behave identically in stand alone expressions like the ones displayed above. They both affect the global environment by creating new name-value pairs for each variable _a_ and _e_.
...
Built-in and user functions may have none, one or more arguments. The arguments are given in between parenthesis, separated by commas, and they may be named or not. An example of a named parameter to a function is `base` in the `log` function: `log(x, base = exp(1))`.

The first argument is a positional one, taking position number one. The second argument is named and receives a value via the `=` operator if the function `log` requires it in its body while executing the function.

If the function is called without a second argument, and `base` is used for computation inside the function, the assignment operator will use the value resulting from evaluating the experession `exp(1)` and give it to `base`, in this regard this is a default value for `base`. If a named argument is given instead as in `log(10, base = 10)`, the value of `base` will be `10`. The important thing is that `log(base = 10, 10)` works just as well because the first argument is named and the second is the first positional argument. A named argument does not count take a position in this respect. 

Another important difference is that the `=` operator for a named function argument does not affect the global environment where the function is created. `=` limits its action to the local scope of the function, in the example: it creates a name-value pair visible only within the function `log`. 
On the other hand, the expression `log(x, base <- exp(1))` would assign the local variable `base` with the result of evaluating `exp(1)` after creating a name-value pair in the global environment: that implies that there will be a global variable `base` with the value of evaluating `exp(1)` outside of the function `log` that will outlive the lifespan of `log` long after it returns with the result of the calculation. In a very concrete way this is a side effect in the global environment that `=` does not present. 

In this last case the value is given to `base` by position because `<-` does not match function parameters by name. Thus if we wrote `log(base <- exp(1), x)` the value of evaluating `exp(1)` would be given to `x` by position, and the value of evaluating `x` from the global environment would be assigned to `base` by position, not really the desired intention.

In summary, to avoid ambiguities and unplanned side effects, when assigning values to variables use `<-` for stand-alone expressions and `=` for function named parameters. Here is an exercise to solifiy these concepts.

```{r named-param, exercise = TRUE}
three <- log(1000, base = 10)
if (exists("base")) print(base) else print("base does not exist outside of log")
unexpected <- log(base <- 10, 1000)
named_params_correct <- log(base = 10, 1000)
still_ok_by_position <- log(1000, base <- 10)

print(paste0("base exists outside of log: base = ", base))
print(paste0("three = ",three))
print(paste0("unexpected = ", unexpected))
print(paste0("named_params_correct = ", named_params_correct))
print(paste0("still_ok_by_position = ", still_ok_by_position))
```
