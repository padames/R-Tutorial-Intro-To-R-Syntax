...
Append the 26th lower case letter of the  ascii character set to the beginning (a rather odd request, point taken!).


```{r append-z-to-head, exercise=TRUE}
y <- letters[1:5]
# append the 26th letter to the head of 'y'

```

```{r append-z-to-head-hint}
y <-  append(letters[26], y)
```
...
These are vectors that can only be populated with predefined values which correspond to categories or levels. For example the vector of musical genres for a sample of music titles from a palylist may be described by:
...
Please note that this syntax is using a concept called a logical mask. It is a form of vector subsetting similar to what we have studied before. Here is an explanation:
...
filter
```
Now subset the dataframe using this logical mask. a `TRUE` value will let the value at that position on each vector of the dataframe to pass as is. A `FALSE` will eliminate it so it does not appear on the result.
...
Remember that section on factors? Well here we go again. When you construct a dataframe from vectors their class will be interpreted by the constructor. Remember R will try to coerce all the elements of the vector to the lowest common denominator to be able to work with them. 
