...
A vector is constructed with the function `c()` that takes a variable number of arguments. Once a vector has been constructed and assigned to a variable `x`, its elements can be extracted with the sub-setting operator `[]`. `x[1]` subsets the vector represented by the name `x` returning another vector with the first  element only.
...
Did you notice the language used to explain sub-setting? `x[1]` returns a vector with only the first element of `x`. In R the sub-setting function returns another vector. Let's verify these statements with R itself:
...
Here is a vector of doubles:

```{r vector-of-doubles, exercise=TRUE, exercise.lines=2}
vd <- c(2.3, -0.54, 1, 10.8)
typeof(vd)
```

And this one contains character strings:

```{r  vector-characters, exercise=TRUE, exercise.lines=2}
vc <- c("Hello", "world", "in", "R!")
typeof(vc)
```

And finally here are two vectors of logical symbols:

```{r vector-logicals, exercise=TRUE, exercise.lines=5}
vl <- c(TRUE, T, T, FALSE, F, F)
typeof(vl)
# the presence of any zero or one coherces all elements to "numeric"
vld <- c(TRUE, T, 1, FALSE, F, 0)
typeof(vld)
```
...
Suppose you have two vectors with values related to the same experiment repeated five consecutive times on two different days. Let's supposed you wanted to know if each of the results from day 1 are greater than or equal those of day 2 in the same order they were obtained. 

```{r comparing-two-vectors, exercise=TRUE, exercise.lines=3}
run_1 <- c(34.2, 28.7, 31.0, 30.7, 29.1)
run_2 <- c(28.5, 35.1, 27.7, 29.1, 29.6)
run_1 >= run_2
```

R is a vectorized language so operators work natively on vector objects.

Logical operators work similarly, remember the AND and OR operators represemted by the `&` and `|` symbols? They have the double ampersand and double pipe versions.

```{r AND-on-vectors-setup}
run_1 <- c(34.2, 28.7, 31.0, 30.7, 29.1)
run_2 <- c(35.1, 28.5, 27.7, 29.1, 29.6)
```

```{r logical-AND-on-vectors, exercise=T, exercise.lines=4, exercise.setup = "AND-on-vectors-setup"}
(val_gt_mean_1 <- run_1 > mean(run_1))
(val_gt_mean_2 <- run_2 > mean(run_2))
val_gt_mean_1 & val_gt_mean_2
val_gt_mean_1 | val_gt_mean_2
```

```{r double-ampersand-on-vectors-setup}
run_1 <- c(34.2, 28.7, 31.0, 30.7, 29.1)
run_2 <- c(35.1, 28.5, 27.7, 29.1, 29.6)
val_gt_mean_1 <- run_1 > mean(run_1)
val_gt_mean_2 <- run_2 > mean(run_2)
...
The doble ampersand operator `&&` always returns a single value, thus when used on two vectors it returns the result of comparing only the first element of each of the two vectors.
...
# new names
names(v)
# finally check the names of the vector with the sum of all the values
names(sum(v))
```
