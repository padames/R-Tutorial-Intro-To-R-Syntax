...
The main goal of this tutorial is to teach patterns of the R language basic syntax. We believe that this knowledge will set you on a positive trajectory at any point on your R language journey. R has specific properties that set it apart from others and that can make it easier to learn for a beginner but a bit confusing at first for people with experience in other computer languages.

By the end of this tutorial you will know what data structures to use to store your data in memory. You will know what basic data types to use according to the data you  have for your specific problem. You will know the syntax to use R as a powerful calculation tool to generate statistical and data insights by sub-setting, filtering, and computing on R native data structures.

You will know how to build complex expressions that compute on R data structures with simplicity, whether they are vectors, lists, or dataframes.

You will know how to search for built-in R functions to help you process the data and achieve your goals. You will also know how to create your own functions if no other function from the built-ins or publicly available R packages meets your needs.



<!-- This introduction to R syntax will allow you to become familiar with the following concepts:

  * How R can be used as a calculator
    - What mathematical operators are
    - How to build complex expressions using operators
  ...
  * How and why to use a dataframe
    - How to create them
    - How to add or delete rows and columns
    - How to subset a dataframe
  * How to create a user defined function
  * Basic conditional change of program execution
-->    
  
R can be easily used to carry out accurate and expressive statistical analysis, complex data processing, classical statistical modelling, machine learning, data visualization, dashboard creation, and many other tasks. 
...
Each result appears on its own line after running the code. This is the same behaviour you would see on a command line if you entered the four lines of code. Go back and prove to yourself that white space between the numbers and the operators has no effect on the result.
...
The `<-` binary operator indicates assignment. The value on the right is assigned to the variable on the left. After line 1 is executed by R the variable `a` stores the value 10 so when the second line is executed b is assigned the result of applying the operator `+` to the value `10` and the value of the variable `a`. As a result of this `b` will be assigned the value `20`.
...
### Assignment from values returned from built-ins 
...
print(0.3, digits = 17)
```
Let's investigate now what the machine's precision to represent a double floating point number:
...
From the table we just saw one could read as follows: all one-dimensional data structures that contain objects of the same type are vectors and thus can be stored in contiguous memory because all the elements require the same space to be stored. A vector of six integer values would be represented graphically as:
...
A vector is constructed with the function `c()` that takes a variable number of arguments. Once a vector has been constructed and assigned to a variable `x` its elements can be extracted with the sub-setting operator `[]` like `x[1]` to subset the vector so it returns a vector with the first  element only.
