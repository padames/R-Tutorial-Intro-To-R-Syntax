...
x[3:5]
```
There are many interesting ways of sub-setting a vector using ranges. For example a negative value indicates the element at that index is to be removed from t he subset of the vector.
...
```{r recycling-a-logical-mask-setup}
x <- c(5, -1, 3, 0, -4, 1)
```
...
```{r recycling-a-logical-mask, exercise=T, exercise.lines=2, exercise.setup="recycling-a-logical-mask-setup"}
...
music_sample <- c("Indie", "Electronic", "Country", "Country", "Indie", "Indie", "Electronic")
class(music_sample)
```
...
attributes(music_sample_2)
```
What if we get a recommendation to listen to a new genre so we add it to the play list before actually listening to any new song. That can be achieved in R by adding a new character string to the vector of levels.

```{r music-sample-genres-as-factors-setup}
music_sample_2 <- factor(c("Indie", "Electronic", "Country", "Country", "Indie", "Indie", "Electronic"))
```

```{r music-sample-adding_new-level, exercise=T, exercise.setup="music-sample-genres-as-factors-setup", exercise.lines=3}
# add a new musical genre (a new level of the categorical variable)
levels(music_sample_2) <- c(levels(music_sample_2), "Classical")
table(music_sample_2)
```

When we start listening to songs of that new genre they will get reckoned appropriately. In R adding a new element to a factor has to be done on original vector because factors cannot be combined or concatenated.

```{r music-sample-genres-levels, exercise=T, exercise.setup="music-sample-genres-as-factors-setup", exercise.lines=4}
# add new entries
music_sample_3 <- factor(c(music_sample, c("Classical", "Indie")))
table(music_sample_3)
```
...
If you need to add a new level to the factor object then there is a form of level assignment to do it, using simple vector linear concatenation with the `c()` function, remember 'nesting vectors'?:

```{r adding-level-to-factor, exercise=T, exercise.lines=8, exercise.setup="tally-factor-setup"}
...
music_sample_genres <- factor(c(genre_data, c("Classical", "Indie", "Indie", "Indie")))
# see the tally
table(music_sample_genres)
```
