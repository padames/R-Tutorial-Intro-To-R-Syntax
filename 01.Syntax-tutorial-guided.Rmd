...
Follow these rules and data frames are an almost ideal data structure to manipulate data for data analysis, as convenient but safer than spreadsheet files, whether in memory or saved as a file to a disk locally or in the cloud. 
...
Under the hood a data frame is implemented as a list of vectors of the same length. The table-like geometry and the constraint of columns consisting of vectors of equal length allow for code optimization on the algorithms to search and select items. Vectors are stored in contiguous memory of constant and fixed size, this gives a data frame some advantages, and they are natively implemented so the `$` and `[` syntax you have already seen can be safely reused for data frames.
...
Try the `str()` function on the data frame we just created. R has a special vocabulary for the rows and columns of a data frame. Each of the vectors in the list are mapped to a variable while each row represents an observation.
...
Because each row on a data frame is associated with an experimental observation of some sort, sometimes an observation may genuinely have missing entries for some of the variables that were simply not measured or are not available in the data set. The function `is.na()` can be used on the whole data frame or on a specific column to return a data frame of logical vectors or a single vector indicating whether an `NA` exists at that position or not: indicated by a `TRUE` or`FALSE`. An exercise will clarify this use:


```{r, detect-NAs-in-a-data-frame, exercise=T, exercise.lines=5}
df <- 
```



Another case is when calculated values do not produce a  valid number, represented in R as  of type `NaN`. 
